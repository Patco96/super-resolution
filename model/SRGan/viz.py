...
def visualize_sr(img, srresnet, srgan_generator, esrgan_generator, halve=False):
    ...
    # Create grid
    margin = 40
    grid_img = Image.new('RGB', (2 * hr_img.width + 3 * margin,
                         2 * hr_img.height + 3 * margin), (255, 255, 255))
    ...
    draw.text(
        xy=[2 * margin + bicubic_img.width + sr_img_srresnet.width /
            2 - text_size[0] / 2, margin - text_size[1] - 5],
    ...
    draw.text(
        xy=[margin + bicubic_img.width / 2 - text_size[0] / 2, 2 *
            margin + sr_img_srresnet.height - text_size[1] - 5],
        text="SRGAN", font=font, fill='black')

    # Place original HR image
    grid_img.paste(hr_img, (2 * margin + bicubic_img.width,
...
def visualize_6(img, srresnet, srgan_generator, esrgan_generator, halve=False):
    ...
    if False:
        ...
        draw.text(xy=[x0 + lr_img.width / 2 - text_size[0] / 2, y0 -
                text_size[1] - 5], text="Low resolution", font=font, fill='black')
    else:
